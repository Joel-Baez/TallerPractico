...
function construirTeclado(){
  ...
  'ABCDEFGHIJKLMNÑOPQRSTUVWXYZ'.split('').forEach(l=>{
    ...
    b.textContent=l;
    b.className='letra';
    b.onclick=()=>probarLetra(l);
    teclado.appendChild(b);
  });
}

function nuevaPalabra(){ return listaPalabras[Math.floor(Math.random()*listaPalabras.length)]; }

function actualizarPantalla(){
  palabraMostrar.textContent = secreta.palabra
    .split('').map(c => c===' ' ? ' ' : letrasElegidas.has(c)? c : '_').join(' ');
  intentos.textContent = maxErrores - errores;
  letrasUsadas.textContent = [...letrasElegidas].filter(l=>!secreta.palabra.includes(l)).join(', ') || '—';
...
function probarLetra(l){
  ...
  const btn = teclado.querySelector(`button:contains(${l})`);
...
function ganaste(){ return secreta.palabra.split('').every(c=> c===' ' || letrasElegidas.has(c)); }
function finJuego(){ return errores>=maxErrores || ganaste(); }
...
document.getElementById('btn-nuevo').onclick = nuevoJuego;
...
document.getElementById('btn-editar').onclick = ()=>{
  entrada.value = listaPalabras.map(w=> w.pista ? `${w.palabra}|${w.pista}` : w.palabra).join(', ');
  entrada.focus();
};

window.addEventListener('keydown', e=>{
  if(/^[a-zA-ZñÑ]$/.test(e.key)) probarLetra(e.key.toUpperCase());
});
